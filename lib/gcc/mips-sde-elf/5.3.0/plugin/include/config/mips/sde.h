...
#define DRIVER_SELF_SPECS						\
  ...
  "%{!EB:%{!EL:%(endian_spec)}}",					\
									\
  /* PIC requires abicalls, which is inconsistent with small data.  */	\
  "%{fPIC|fpic:-mabicalls -G0}",					\
									\
...
#ifdef __PIC__
/* Since the bits of the _init and _fini function are spread across
   many object files, each potentially with its own GP, we must assume
   we need to load our GP.  We don't preserve $gp or $ra, since each
   init/fini chunk is supposed to initialize $gp, and crti/crtn
   take care of preserving $ra and, when appropriate, $gp.  */
